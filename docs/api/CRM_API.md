...
**`params` для `type: 'search'`:** обязательно непустой **`queries`**: `string[]`. Нужен хотя бы один BD: **`bdAccountId`** (uuid) и/или **`accountIds`**: uuid[] (1–10). При нескольких аккаунтах discovery-loop может переключаться при **429/502/503** от bd-accounts (см. [TELEGRAM_PARSE_FLOW.md](../domain/TELEGRAM_PARSE_FLOW.md)). Опционально: `searchType` (`groups` \| `channels` \| `all`), `limitPerQuery` (1–100). В UI (**Contact discovery** → новый поиск) при выборе одного аккаунта уходит `bdAccountId`, при нескольких — **`accountIds`**.
