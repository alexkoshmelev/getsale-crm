...
2. **Апдейты не доходят до сервера**  
  ...
  - Соединение «засыпает» — keepalive (GetState раз в 30 секунд) должен это предотвращать.
...
## Proxy support contract

- Поддерживаемый тип прокси для Telegram-подключений: **SOCKS5**.
- HTTP/HTTPS proxy для auth/connect потоков не поддерживается и отклоняется.
- Рекомендуем использовать только SOCKS5 в onboarding и в настройках BD-аккаунта.
