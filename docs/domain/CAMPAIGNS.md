...
Цели, статус реализации и связь с продуктом. Детальный статус и приоритеты — [ROADMAP.md](../ROADMAP.md).
...
**Не сделано / бэклог:** статус campaign «completed» при завершении всех участников; rate limiting по каналу в worker; AI-персонализация ({{ai.personalize}} — заглушка); оптимальное время отправки (модель). См. также [COMPETITOR_ANALYSIS.md](../product/COMPETITOR_ANALYSIS.md) и [OUTREACH_BEST_PRACTICES.md](OUTREACH_BEST_PRACTICES.md).
