...
**Общее:** `SERVICE_HTTP_TIMEOUT_MS`, `SERVICE_HTTP_RETRIES`, `SERVICE_HTTP_RETRY_DELAY_MS`, `SERVICE_HTTP_CB_THRESHOLD`, `SERVICE_HTTP_CB_RESET_MS` — см. [DEPLOYMENT.md](DEPLOYMENT.md).

**Метрики (B1):** при создании клиента передаётся `metricsRegistry` из `createServiceApp` — на `/metrics` публикуются `inter_service_http_requests_total` и `inter_service_http_circuit_reject_total` (см. [DEPLOYMENT.md](DEPLOYMENT.md) → Prometheus).
...
- [MIGRATION_TO_TARGET_ARCHITECTURE.md](MIGRATION_TO_TARGET_ARCHITECTURE.md) — фаза B1  
- [CURRENT_SYSTEM_AS_IS.md](CURRENT_SYSTEM_AS_IS.md) — строки таблицы сервисов  
