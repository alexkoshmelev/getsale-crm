...
Существующие эндпоинты sync-chats можно оставить для обратной совместимости или постепенно перевести UI на «только папки».

**Гибридная синхронизация истории:** sync-start загружает только **одну страницу сообщений на чат** (по умолчанию 100, `SYNC_INITIAL_MESSAGES_PER_CHAT`). Синхронизация завершается быстро; пользователь сразу видит чаты с последними сообщениями. Старая история подгружается **по требованию**: при скролле вверх в конкретном чате вызывается `POST load-older-history`, затем сообщения читаются из БД. Так избегаем долгой начальной синхронизации и равномерно распределяем нагрузку на Telegram API.
