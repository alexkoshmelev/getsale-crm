...
async function start() {
  ...
  app.get('/ready', async (request, reply) => {
    try {
      await redis.ping();
      return { ready: true, service: 'gateway', checks: { redis: 'ok' } };
    } catch {
      reply.code(503);
      return { ready: false, service: 'gateway', checks: { redis: 'error' } };
    }
  });
  ...
  log.info({ message: `Gateway running on port ${PORT}` });

  const shutdown = async () => {
    log.info({ message: 'Gateway shutting down' });
    await app.close();
    redis.disconnect();
...
start().catch((err) => {
  log.error({ message: 'Gateway failed to start', error: String(err) });
  process.exit(1);
});
